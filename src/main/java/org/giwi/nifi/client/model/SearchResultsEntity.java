/*
 * Apache NiFi REST API
 * REST API definition for Apache NiFi web services
 *
 * The version of the OpenAPI document: 2.9.0
 * Contact: dev@nifi.apache.org
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package org.giwi.nifi.client.model;

import java.util.Objects;
import java.util.Arrays;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonTypeName;
import com.fasterxml.jackson.annotation.JsonValue;
import org.giwi.nifi.client.model.SearchResultsDTO;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;
import com.fasterxml.jackson.annotation.JsonTypeName;

/**
 * SearchResultsEntity
 */
@JsonPropertyOrder({
  SearchResultsEntity.JSON_PROPERTY_SEARCH_RESULTS_D_T_O
})
@javax.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", date = "2026-04-27T21:33:06.905180638+02:00[Europe/Paris]", comments = "Generator version: 7.14.0")
public class SearchResultsEntity {
  public static final String JSON_PROPERTY_SEARCH_RESULTS_D_T_O = "searchResultsDTO";
  @javax.annotation.Nullable
  private SearchResultsDTO searchResultsDTO;

  public SearchResultsEntity() {
  }

  public SearchResultsEntity searchResultsDTO(@javax.annotation.Nullable SearchResultsDTO searchResultsDTO) {
    
    this.searchResultsDTO = searchResultsDTO;
    return this;
  }

  /**
   * Get searchResultsDTO
   * @return searchResultsDTO
   */
  @javax.annotation.Nullable
  @JsonProperty(JSON_PROPERTY_SEARCH_RESULTS_D_T_O)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)

  public SearchResultsDTO getSearchResultsDTO() {
    return searchResultsDTO;
  }


  @JsonProperty(JSON_PROPERTY_SEARCH_RESULTS_D_T_O)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setSearchResultsDTO(@javax.annotation.Nullable SearchResultsDTO searchResultsDTO) {
    this.searchResultsDTO = searchResultsDTO;
  }


  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    SearchResultsEntity searchResultsEntity = (SearchResultsEntity) o;
    return Objects.equals(this.searchResultsDTO, searchResultsEntity.searchResultsDTO);
  }

  @Override
  public int hashCode() {
    return Objects.hash(searchResultsDTO);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class SearchResultsEntity {\n");
    sb.append("    searchResultsDTO: ").append(toIndentedString(searchResultsDTO)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

