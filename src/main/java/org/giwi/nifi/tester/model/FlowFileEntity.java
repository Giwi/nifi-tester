/*
 * Apache NiFi REST API
 * REST API definition for Apache NiFi web services
 *
 * The version of the OpenAPI document: 2.9.0
 * Contact: dev@nifi.apache.org
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package org.giwi.nifi.tester.model;

import java.util.Objects;
import java.util.Arrays;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonTypeName;
import com.fasterxml.jackson.annotation.JsonValue;
import org.giwi.nifi.tester.model.FlowFileDTO;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;
import com.fasterxml.jackson.annotation.JsonTypeName;

/**
 * FlowFileEntity
 */
@JsonPropertyOrder({
  FlowFileEntity.JSON_PROPERTY_FLOW_FILE
})
@javax.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", date = "2026-04-27T21:33:06.905180638+02:00[Europe/Paris]", comments = "Generator version: 7.14.0")
public class FlowFileEntity {
  public static final String JSON_PROPERTY_FLOW_FILE = "flowFile";
  @javax.annotation.Nullable
  private FlowFileDTO flowFile;

  public FlowFileEntity() {
  }

  public FlowFileEntity flowFile(@javax.annotation.Nullable FlowFileDTO flowFile) {
    
    this.flowFile = flowFile;
    return this;
  }

  /**
   * Get flowFile
   * @return flowFile
   */
  @javax.annotation.Nullable
  @JsonProperty(JSON_PROPERTY_FLOW_FILE)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)

  public FlowFileDTO getFlowFile() {
    return flowFile;
  }


  @JsonProperty(JSON_PROPERTY_FLOW_FILE)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setFlowFile(@javax.annotation.Nullable FlowFileDTO flowFile) {
    this.flowFile = flowFile;
  }


  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    FlowFileEntity flowFileEntity = (FlowFileEntity) o;
    return Objects.equals(this.flowFile, flowFileEntity.flowFile);
  }

  @Override
  public int hashCode() {
    return Objects.hash(flowFile);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class FlowFileEntity {\n");
    sb.append("    flowFile: ").append(toIndentedString(flowFile)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

